...
(function () {
  ...
  const RESERVED_CLASSES = ['xs', 'sm', 'lg', 'fw', 'ul', 'li', 'border', 'pull-left', 'pull-right', 'spin', 'pulse', 'rotate-90', 'rotate-180', 'rotate-270', 'flip-horizontal', 'flip-vertical', 'stack', 'stack-1x', 'stack-2x', 'inverse', 'layers', 'layers-text', 'layers-counter'].concat(oneToTen.map(n => `${n}x`)).concat(oneToTwenty.map(n => `w-${n}`));
  ...
  function update(newConfig) {
    ...
    const _params$asNewDefault = params.asNewDefault;


    const asNewDefault = _params$asNewDefault === undefined ? false : _params$asNewDefault;

    const validKeys = Object.keys(config);
    const ok = asNewDefault ? function (k) {
      return ~validKeys.indexOf(k) && !~initialKeys.indexOf(k);
    } : function (k) {
      return ~validKeys.indexOf(k);
    };

    Object.keys(newConfig).forEach((configKey) => {
  ...
  function joinAttributes(attributes) {
    return Object.keys(attributes || {}).reduce((acc, attributeName) => `${acc}${attributeName}="${htmlEscape(attributes[attributeName])}" `, '').trim();
  }

  function joinStyles(styles) {
    return Object.keys(styles || {}).reduce((acc, styleName) => `${acc}${styleName}: ${styles[styleName]};`, '');
  }
  ...
  function transformForCss(_ref2) {
    ...
    const width = _ref2$width === undefined ? UNITS_IN_GRID : _ref2$width;


    const _ref2$height = _ref2.height;


    const height = _ref2$height === undefined ? UNITS_IN_GRID : _ref2$height;


    const _ref2$startCentered = _ref2.startCentered;


    const startCentered = _ref2$startCentered === undefined ? false : _ref2$startCentered;
  ...
  const makeIconMasking = function (_ref) {
    ...
    const trans = transformForSvg({ transform, containerWidth: maskWidth, iconWidth: mainWidth });
  ...
  const makeIconStandard = function (_ref) {
    ...
    if (transformIsMeaningful(transform)) {
      const trans = transformForSvg({ transform, containerWidth: main.width, iconWidth: main.width });
      children.push({
        tag: 'g',
        attributes: _extends({}, trans.outer),
        children: [{
  ...
  function makeInlineSvgAbstract(params) {
    ...
    const watchable = _params$watchable === undefined ? false : _params$watchable;
    ...
    const attrClass = [config.replacementClass, iconName ? `${config.familyPrefix}-${iconName}` : '', widthClass].concat(extra.classes).join(' ');
  ...
  function makeLayersTextAbstract(params) {
    ...
    const attributes = _extends({}, extra.attributes, title ? { title } : {}, {
    ...
    if (transformIsMeaningful(transform)) {
      styles.transform = transformForCss({
        transform, startCentered: true, width, height,
      });
 ...
 */
  ...
  const build = function build() {
    const lookup = function lookup(reducer) {
      return reduce(styles$2, (o, style, prefix) => {
  ...
  function toHtml(abstractNodes) {
    ...
    const attributes = _abstractNodes$attrib === undefined ? {} : _abstractNodes$attrib;


    const _abstractNodes$childr = abstractNodes.children;


    const children = _abstractNodes$childr === undefined ? [] : _abstractNodes$childr;
  ...
  function searchPseudoElements(root) {
    ...
    disableObservation(() => {
      toArray(root.querySelectorAll('*')).forEach((node) => {
        [':before', ':after'].forEach((pos) => {
          ...
          let pseudoElement = children.filter(c => c.getAttribute(DATA_FA_PSEUDO_ELEMENT) === pos)[0];
  ...
  function onTree(root) {
    ...
    const prefixesDomQuery = [`.${LAYERS_TEXT_CLASSNAME}:not([${DATA_FA_I2SVG}])`].concat(prefixes.map(p => `.${p}:not([${DATA_FA_I2SVG}])`)).join(', ');
  ...
  function findIconDefinition(params) {
    ...
    const prefix = _params$prefix === undefined ? 'fa' : _params$prefix;
  ...
  const dom = {
    i2svg: function i2svg() {
      ...
      if (IS_DOM) {
        ...
        const node = _params$node === undefined ? DOCUMENT : _params$node;


        const _params$callback = params.callback;


        const callback = _params$callback === undefined ? function () {} : _params$callback;
  ...
  const icon = resolveIcons(function (iconDefinition) {
    ...
    const transform = _params$transform === undefined ? meaninglessTransform : _params$transform;


    const _params$symbol = params.symbol;


    const symbol = _params$symbol === undefined ? false : _params$symbol;


    const _params$mask = params.mask;


    const mask = _params$mask === undefined ? null : _params$mask;


    const _params$title = params.title;


    const title = _params$title === undefined ? null : _params$title;


    const _params$classes = params.classes;


    const classes = _params$classes === undefined ? [] : _params$classes;


    const _params$attributes = params.attributes;


    const attributes = _params$attributes === undefined ? {} : _params$attributes;


    const _params$styles = params.styles;


    const styles = _params$styles === undefined ? {} : _params$styles;
  ...
  const text = function text(content) {
    ...
    const transform = _params$transform2 === undefined ? meaninglessTransform : _params$transform2;


    const _params$title2 = params.title;


    const title = _params$title2 === undefined ? null : _params$title2;


    const _params$classes2 = params.classes;


    const classes = _params$classes2 === undefined ? [] : _params$classes2;


    const _params$attributes2 = params.attributes;


    const attributes = _params$attributes2 === undefined ? {} : _params$attributes2;


    const _params$styles2 = params.styles;


    const styles = _params$styles2 === undefined ? {} : _params$styles2;


    return apiObject({ type: 'text', content }, () => {
      ensureCss();
